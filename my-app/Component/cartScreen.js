...
const Cart = () => {
  ...
  const handleRemoveFromCart = async (productId) => {
    try {
      ...
      Alert.alert('Item Removed', 'The item has been removed from the cart.', [{ text: 'OK' }]);
    } catch (error) {
      console.error('There was an error removing item from cart:', error);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Nav />
      <Text style={{ fontWeight: 'bold', alignSelf: 'center', fontSize: 20, letterSpacing: 6 }}>CART</Text>
      ...
      <View style={styles.AddToBasket}>
        ...
        <Text style={{ color: "white", marginLeft: 10, fontSize: 22 }}>CART</Text>
      </View>
    </SafeAreaView>
  );
};
